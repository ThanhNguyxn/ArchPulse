...
function generateDashboardHTML(
  ...
  const graphNodes = generateGraphNodes(analysis);
  const graphLinks = generateGraphLinks(analysis);
...
/**
 * Generate nodes for D3 force-directed graph
 */
function generateGraphNodes(analysis: ArchitectureAnalysis): GraphNode[] {
  const nodes: GraphNode[] = [];
  const layerMap = new Map<string, { name: string; color: string }>();
  
  // Build layer lookup
  for (const layer of analysis.layers) {
    for (const modulePath of layer.modules) {
      layerMap.set(modulePath, { name: layer.name, color: layer.color });
    }
  }

  // Get top modules by connections (limit to 40 for performance)
  const moduleScores: Array<{ path: string; score: number }> = [];
  for (const [path, node] of analysis.graph.nodes) {
    const outgoing = analysis.graph.edges.filter(e => e.from === path).length;
    const incoming = analysis.graph.edges.filter(e => e.to === path).length;
    moduleScores.push({ path, score: outgoing + incoming });
  }
  ...
  for (const modulePath of topModules) {
    const node = analysis.graph.nodes.get(modulePath);
    if (node) {
      const layerInfo = layerMap.get(modulePath) || { name: 'Other', color: '#6b7280' };
      nodes.push({
        id: modulePath,
        name: node.name,
        layer: layerInfo.name,
        color: layerInfo.color,
        deps: moduleScores.find(m => m.path === modulePath)?.score || 0
      });
    }
  }
...
/**
 * Generate links for D3 force-directed graph
 */
function generateGraphLinks(analysis: ArchitectureAnalysis): GraphLink[] {
  const links: GraphLink[] = [];
  
  // Get top modules for filtering
  const moduleScores: Array<{ path: string; score: number }> = [];
  for (const [path] of analysis.graph.nodes) {
    const outgoing = analysis.graph.edges.filter(e => e.from === path).length;
    const incoming = analysis.graph.edges.filter(e => e.to === path).length;
    moduleScores.push({ path, score: outgoing + incoming });
  }
  ...
  // Only include edges between top modules
  for (const edge of analysis.graph.edges) {
    if (topModules.has(edge.from) && topModules.has(edge.to)) {
      links.push({
        source: edge.from,
        target: edge.to
      });
