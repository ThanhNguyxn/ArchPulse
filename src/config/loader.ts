...
export const DEFAULT_CONFIG: ArchPulseConfig = {
  ...
  extensions: ['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.pyw'],
};
...
function parseGroupingRules(raw: unknown): GroupingRule[] {
  ...
  return raw
    ...
    .filter(item => typeof item.pattern === 'string' && typeof item.label === 'string')
    .map(item => ({
...
function parseConfig(raw: unknown): Partial<ArchPulseConfig> {
  ...
  if (Array.isArray(obj.ignore)) {
    result.ignore = obj.ignore.filter((item): item is string => typeof item === 'string');
  }
 ...
 */
function mergeConfig(base: ArchPulseConfig, override: Partial<ArchPulseConfig>): ArchPulseConfig {
  return {
